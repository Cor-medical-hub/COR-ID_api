...
def upgrade() -> None:
    ...
    op.create_unique_constraint(op.f('uq_patients_patient_cor_id'), 'patients', ['patient_cor_id'])


    # 6. Создаем НОВЫЙ внешний ключ от `patients.user_id` к `users.id`.
    op.create_foreign_key(op.f('fk_patients_user_id_users_id'), 'patients', 'users', ['user_id'], ['id'])
...
def downgrade() -> None:
    ...
    op.drop_constraint(op.f('uq_patients_user_id'), 'patients', type_='unique') # Удаляем unique constraint на user_id
    op.drop_constraint(op.f('fk_patients_user_id_users_id'), 'patients', type_='foreignkey')
