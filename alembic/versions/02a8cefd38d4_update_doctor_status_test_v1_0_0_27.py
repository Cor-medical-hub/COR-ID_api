...
from cor_pass.database.models import (
    Doctor_Status,
)  # Замените your_app.models на фактический путь
...
def upgrade():
    ...
    op.add_column(
        "doctors",
        sa.Column(
            "status", new_doctor_status, nullable=False, server_default="pending"
        ),
    )
...
def downgrade():
    ...
    op.add_column(
        "doctors",
        sa.Column(
            "status", old_doctor_status, nullable=False, server_default="pending"
        ),
    )
