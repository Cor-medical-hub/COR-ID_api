...
from cor_pass.schemas import DeviceAccessResponse, DeviceResponse, DeviceRegistration, GenerateManufacturedDevices, GrantDeviceAccess
...
async def device_websocket_endpoint(websocket: WebSocket, 
    ...
    try:
        ...
        device_id_str = str(device.id) # Преобразуем ID устройства в строку для ключей словарей
        ...
        await websocket.close(code=status.HTTP_500_INTERNAL_SERVER_ERROR, reason="Internal Server Error")
...
async def activate_device(registration: DeviceRegistration, current_user: User = Depends(auth_service.get_current_user), db: AsyncSession = Depends(get_db)):
    manufactured_device = await repository_devices.get_manufactured_device_by_token(db, registration.device_token)
    if not manufactured_device:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Устройство не найдено или уже активировано"
        )
    jwt_token = await auth_service.create_device_jwt(device_id=manufactured_device.id, user_id=current_user.cor_id, expires_delta=10)
    ...
    await repository_devices.update_manufactured_device_status(db, manufactured_device, DeviceStatus.ACTIVATED)

    return DeviceResponse (token=db_device.token, device_name=db_device.name, user_id=db_device.user_id)
...
async def websocket_endpoint(websocket: WebSocket, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not token:
            await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="Токен отсутствует")
            return
        ...
        await websocket.send_text(f"Успешная аутентификация устройства (ID: {device.id}, Name: {device.name})")

        while True:
            data = await websocket.receive_text()
            await websocket.send_text(f"Сообщение от устройства (ID: {device.id}, Name: {device.name}) получено: {data}")
...
async def share_device(
    ...
    device = await repository_devices.get_device_by_id(device_id=access_data.device_id, db=db)
    if device:
    ...
    user_to_grant_access = await repository_users.get_user_by_corid(db=db, cor_id=access_data.user_id)
    ...
    existing_access = await repository_devices.get_device_access(
        db=db, device_id=access_data.device_id, granting_user_id=current_user.cor_id, accessing_user_id=access_data.user_id
    )
    ...
    if existing_access:
        ...
        new_access = await repository_devices.create_device_access(
            db, access_data.device_id, current_user.cor_id, access_data.user_id, access_data.access_level
        )
        return new_access
...
async def get_device_data(
    device: Device = Depends(lambda device_id, current_user, db: auth_service.verify_device_access(
        device_id=device_id,
