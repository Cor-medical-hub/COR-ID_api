...
@router.post("/samples/{sample_id}/cassette", 
            #  response_model=Cassette
             )
async def create_cassette_for_sample(sample_id: str, num_cassettes: int = 1, num_glasses_per_cassette: int = 1, db: AsyncSession = Depends(get_db)):
    return await cassette_service.create_cassette(db=db, sample_id=sample_id, num_cassettes=num_cassettes, num_glasses_per_cassette=num_glasses_per_cassette)


@router.get("/{cassette_id}", response_model=Cassette)
