...
from cor_pass.services.access import user_access, doctor_access, admin_access


router = APIRouter(prefix="/printing_devices", tags=["Printing Devices"])


@router.post("/", dependencies=[Depends(admin_access)] ,response_model=ResponcePrintingDevice, status_code=201)
async def create_new_printing_device(
    ...
    existing_device = await get_printing_device_by_device_identifier(device_identifier=body.device_identifier, db=db)
    if existing_device:
...
@router.get("/all", dependencies=[Depends(admin_access)], response_model=List[ResponcePrintingDevice])
...
@router.get("/{printing_device_id}", dependencies=[Depends(admin_access)], response_model=ResponcePrintingDevice)
...
@router.put("/{printing_device_id}", dependencies=[Depends(admin_access)], response_model=ResponcePrintingDevice)
async def update_printing_device_by_id(
    ...
    printing_device = await update_printing_device(uuid=printing_device_id, body=body, db=db)
    if not printing_device:
        raise HTTPException(status_code=404, detail="Printing device not found")
    return printing_device


@router.delete("/{printing_device_id}", dependencies=[Depends(admin_access)], status_code=204)
