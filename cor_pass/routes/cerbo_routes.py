...
from cor_pass.repository.cerbo_service import (
    ...
    delete_schedule,
    get_all_schedules,
    get_device_measurements_paginated,
    get_modbus_client,
    get_schedule_by_id,
...
schedules_storage = {"schedule_enabled": True, "periods": []}
...
async def get_battery_status(request: Request):
    try:
        ...
        if result.isError():
            raise HTTPException(
                status_code=500, detail="Ошибка чтения регистров батареи"
            )
...
async def get_inverter_power_status(request: Request):
    ...
    try:
        ...
        for name, addr in reg_map.items():
            ...
            if res.isError():
                raise HTTPException(
                    status_code=500, detail=f"Ошибка чтения регистров {addr}-{addr+1}"
                )
...
async def get_ess_ac_status(request: Request):
    ...
    try:
        ...
        if result.isError():
            raise HTTPException(
                status_code=500, detail="Ошибка чтения AC регистров ESS"
            )
        ...
        response = {
            "input": {
                ...
                "powers": {
                    ...
                    "total": get_value("input_power_l1")
                    + get_value("input_power_l2")
                    + get_value("input_power_l3"),
                },
            },
            "output": {
                "voltages": {
...
async def get_vebus_status(request: Request):
    ...
    try:
        ...
        if result.isError():
            raise HTTPException(
                status_code=500, detail="Ошибка чтения регистров VE.Bus"
            )
...
async def set_vebus_soc(control: VebusSOCControl, request: Request):
    ...
    try:
        ...
        await client.write_register(
            address=2901, value=scaled_value, slave=100  # адрес регистра VE.Bus SoC
        )
        error_count = 0
        return {"status": "ok"}
    except Exception as e:
...
async def set_ess_advanced_setpoint_fine(control: EssAdvancedControl, request: Request):
    ...
    try:
        ...
        if register_value < 0:
            register_value = (
                1 << 16
            ) + register_value  # Преобразование в 16-битное представление

        # Проверяем, что значение вписывается в int16
        if register_value < 0 or register_value > 65535:
            raise HTTPException(
                status_code=400, detail="Значение выходит за допустимые пределы"
            )

        # Записываем значение в регистр 2703
        await client.write_register(address=2703, value=register_value, slave=slave)
        error_count = 0
        logging.info(
            f"✅ Установлено AC Power Setpoint Fine: {control.ac_power_setpoint_fine} W (регистр 2703 = {register_value})"
        )
...
async def get_ess_settings(request: Request):
    ...
    try:
        ...
        result = await client.read_holding_registers(
            start_address, count=count, slave=slave
        )
        if result.isError():
            raise HTTPException(
                status_code=500, detail="Ошибка чтения регистров ESS Settings"
            )
...
async def get_ess_advanced_settings(request: Request):
    ...
    try:
        ...
        start_main = 2700
        count_main = 13
        result_main = await client.read_input_registers(
            start_main, count=count_main, slave=slave
        )
        if result_main.isError() or not hasattr(result_main, "registers"):
            raise HTTPException(
                status_code=500, detail="Ошибка чтения регистров 2700–2712"
            )
        ...
        def safe_main(idx):
            return (
                r_main[idx - start_main] if (idx - start_main) < len(r_main) else None
            )
        ...
        # Формируем результат
        result_data = {
            "ac_power_setpoint": safe_main(
                2700
            ),  # Просто читаем значение регистра 2700
            "max_charge_percent": safe_main(2701),
            "max_discharge_percent": safe_main(2702),
            "ac_power_setpoint_fine": (
                s16(safe_main(2703)) * 100 if safe_main(2703) is not None else None
            ),
            "max_discharge_power": (
                s16(safe_main(2704)) * 10 if safe_main(2704) is not None else None
            ),
            "dvcc_max_charge_current": s16(safe_main(2705)),
            "max_feed_in_power": (
                s16(safe_main(2706)) * 10 if safe_main(2706) is not None else None
            ),
            "overvoltage_feed_in": safe_main(2707),
            "prevent_feedback": safe_main(2708),
            "grid_limiting_status": safe_main(2709),
            "max_charge_voltage": (
                safe_main(2710) / 10.0 if safe_main(2710) is not None else None
            ),
...
async def test_dynamic_ess_registers(
    ...
    for reg in range(start, end + 1):
        try:
            res = await client.read_holding_registers(
                address=reg, count=1, slave=unit_id
            )
            if res.isError():
...
async def read_measurements(
    ...
    page_size: int = Query(
        10, ge=1, le=100, description="Количество элементов на странице (от 1 до 100)"
    ),
    object_name: Optional[str] = Query(None, description="Фильтр по имени объекта"),
    start_date: Optional[datetime] = Query(
        None,
        description="Начальная дата измерения (ISO 8601, например '2023-01-01T00:00:00')",
    ),
    end_date: Optional[datetime] = Query(
        None,
        description="Конечная дата измерения (ISO 8601, например '2023-12-31T23:59:59')",
    ),
    ...
    try:
        ...
        for period in schedule_data.periods:
            ...
            existing_index = next(
                (
                    i
                    for i, p in enumerate(schedules_storage["periods"])
                    if p["id"] == period.id
                ),
                None,
            )
        ...
        logger.info(
            f"Период(ы) расписания сохранены. Активно: {schedule_data.scheduleEnabled}"
        )
        logger.info(f"Количество полученных периодов: {len(schedule_data.periods)}")
        logger.debug(
            "Детали периодов:",
            json.dumps([p.dict() for p in schedule_data.periods], indent=2),
        )
...
async def create_energetic_schedule(
    schedule_data: EnergeticScheduleCreate, db: AsyncSession = Depends(get_db)
):
...
async def get_energetic_schedule(schedule_id: str, db: AsyncSession = Depends(get_db)):
...
async def get_all_energetic_schedules_api(db: AsyncSession = Depends(get_db)):
