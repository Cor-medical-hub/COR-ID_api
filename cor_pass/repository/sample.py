...
async def create_sample(db: AsyncSession, case_id: str, sample_in: SampleCreate) -> SampleModelScheema:
    ...
    samples_result = await db.execute(
        ...
        .order_by(db_models.Sample.sample_number)  # Важно для определения последнего номера
    )
    ...
    if existing_sample_numbers:
        ...
        try:
            ...
            if last_index < len(ascii_uppercase) - 1:
                ...
                next_sample_number = f"Z{len(existing_sample_numbers) + 1 - len(ascii_uppercase)}"
        except ValueError:
...
async def delete_sample(db: AsyncSession, sample_id: str) -> SampleModelScheema | None:
    ...
    result = await db.execute(select(db_models.Sample).where(db_models.Sample.id == sample_id))
