...
async def get_manufactured_device_by_token(db: AsyncSession, token: str) -> ManufacturedDevice | None:
...
async def create_device(db: AsyncSession, token: str, name: str, user_id: str, serial_number: str, id: str) -> Device:
    db_device = Device(id=id, token=token, name=name, user_id=user_id, serial_number=serial_number)
    db.add(db_device)
    await db.commit()
...
async def update_manufactured_device_status(db: AsyncSession, manufactured_device: ManufacturedDevice, status: DeviceStatus):
...
async def create_manufactured_devices_bulk(db: AsyncSession, count: int):
    ...
    for _ in range(count):
        ...
        serial_number = str(uuid.uuid4().hex[:12])  # Генерируем короткий уникальный серийный номер
